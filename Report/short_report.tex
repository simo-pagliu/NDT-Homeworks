...
Helium concentrations after 1 and 2 years of operation are $67$~ppm and $122$~ppm, respectively. These values match those reported in the literature (Olander). A more accurate evaluation would require calculating helium per displacements-per-atom (ppm/DPA), but this information is not available.

\subsubsection*{Impact on Design and Operation}

Helium embrittlement weakens the cladding material, and its effects should be taken into account in the design phase to ensure the cladding can operate safely over time.
